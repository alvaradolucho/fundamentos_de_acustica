...
#@output.capture()
def get_frequency(m, k):
    freq = ( 1 / (2 * np.pi)) * np.sqrt(k / m)
    print( freq )


# output = widgets.interactive_output(get_frequency, {'m': m_slider, 'k': k_slider})
...
# f = 1
# omega = 2 * np.pi * f
omega = np.sqrt(k_slider.value / m_slider.value)
phi = 0.0
